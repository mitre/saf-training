...
:::warning it vs. its
Important! Just like in English grammar, pay attention to the difference between the thing (it) and the possessive word (its). `it` would be followed by something like `should`. `its` would be followed by some attribute of the thing, like `signal`. Here are two examples:
`it { should be_owned_by 'root' }` where `it` is a `file`.
`its("signal") { should eq "on" }` where `signal` is an attribute that belongs to a specific config resource.
:::
