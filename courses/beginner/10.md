...
#### Combining Inputs with profile dependencies to further enhance Overlay profiles

In addition to customizing the overlay based on dependent controls, one can tailor further by leveraging inputs as discussed earlier. Let's say your environment uses `root` not `admin`, so the input for `nginx-shell-access` control can be updated to measure accordingly.
