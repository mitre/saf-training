...
Good InSpec tests will use resources, many of which are already built in, to easily describe some part of the sysyem. Rember the `file` resource as an example from the previous section. Tests should also use matchers to implement the logic check of an expected result. The previous example used `be_directory` as a matcher. 

:::tip Make Writing InSpec Easer with Builtin Resources
...
The [InSpec shell](https://www.inspec.io/docs/reference/shell/), which enables you to explore InSpec interactively. In this part, you'll use the InSpec shell to discover which resources you can use to test your NGINX configuration.
