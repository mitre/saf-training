...
index: 4
icon: page
title: 4. Exploring the InSpec Executable
author: Aaron Lippold
headerDepth: 3
---

## Exploring the InSpec Executable

Before we test our NGINX configuration, let's take a look at what we can do with the InSpec executable itself.

### The InSpec `exec` command

First and foremost, you need to know how to use `inspec exec` to run a profile against a system. Take a look at our breakdown on [How To Run InSpec](https://mitre.github.io/saf-training/courses/user/06.html) for the MITRE SAF User course for more information.

### The InSpec Shell 

Now we know how to use the `inspec exec` command to actually run our test profiles. Next we want to discuss how we can use InSpec's existing documentation and features to support test development.

To write good tests, we should plan which resources and matchers we'll need. InSpec features dozens of resources and matchers that come predefined in the language. These resources are a core benefit to using InSpec because they allow you to leverage existing stable code to write simple and consistent tests.

- You can [explore the InSpec documentation](https://www.inspec.io/docs/) to see which resources and matchers are available, including descriptions of their attributes and examples of their use.
- You can even [examine the source code](https://github.com/inspec/inspec) to see what's available. For example, you can see how `file` and other InSpec resources are implemented.
...
When writing profiles, you are encouraged to refer back to the docs for resources early and often. They often give you an example of how to write a test that matches your current problem.

You also have an excellent source of information in the [InSpec shell](https://www.inspec.io/docs/reference/shell/), which enables you to explore InSpec interactively. In this part, you'll use the InSpec shell to discover which resources you can use to test your NGINX configuration.

You're not required to use the InSpec shell to develop your profiles, but some users (including this course's instructors) find the InSpec shell to be a useful way to get immediate feedback and explore what's available. You can also use the InSpec shell to debug your tests; the shell lets you write and execute `describe` blocks in-line.
