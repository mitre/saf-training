...
::: details DbProtect-to-OHDF Schema Mapping
```typescript
{
  platform: {
    name: 'Heimdall Tools',
...
::: details Remaining DbProtect-to-OHDF Schema Mapping
```typescript
{
  profiles: [
    {
...
```
Note that in this example, we must call the path with `data.[0]`, since even though the Policy is the same fo all data rows, we still must specify an element to extract the data from. 
