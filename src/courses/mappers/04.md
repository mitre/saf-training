...
An OHDF mapper is a mapper that allows the underlying conversion infrastructure found in OHDF Converters to correlate certain objects or values from one overarching security data format to another overarching security data format. In the case of the SAF tool suite, these mappers allow for the conversion of any given security service format to HDF (\*-to-HDF) and vice versa (HDF-to-\*) using the tools provided by the existing conversion infrastructure.

OHDF mappers are built upon the schemas that define the involved security data formats, and the mechanics of the mappers revolve around correlating each schema's fields as accurately as possible.
