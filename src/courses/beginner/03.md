...
First, run `tree` from the directory that contains your `my_nginx` profile to see what's in the profile.
...
- The `controls` directory contains files that implement the InSpec tests.
...
- `control` (line 10) is followed by the control's name. Each control in a profile has a unique name.
