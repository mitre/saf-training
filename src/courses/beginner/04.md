...
Use the InSpec executable with the command `inspec exec` to run a profile against a system. The generic command is below, but take a look at our breakdown on [How To Run InSpec](../user/06.md) for the MITRE SAF User course for more information.
...
Inspec is a [Domain Specific Language (DSL)](https://www.jetbrains.com/mps/concepts/domain-specific-languages/) on top of Ruby. In other words, InSpec is built on the Ruby programming language. For example, InSpec matchers are implemented as Ruby methods. 
:::
...
```sh
1. NGINX should be installed as 1.27.0 or later.
...
In the next section, we will start writing controls for `my_nginx` profile, but first let's see what resources are available to us to model the NGINX webserver.
