...
To learn how you can use `generate xccdf_benchmark2inspec_stub` or any other `saf` cli command, go to the [saf-cli homepage](https://saf-cli.mitre.org). The table of contents at the top of the page is a great way to see individual commands -- the InSpec Profile generator is [here](https://saf-cli.mitre.org/#inspec-profile). 

You can also use use the help commands for the SAF CLI without having to go to the Internet. For example, the help text for `generate xccdf_benchmark2inspec_stub` is below.
...
```sh
USAGE
  $ saf generate xccdf_benchmark2inspec_stub saf generate xccdf_benchmark2inspec_stub -i <stig-xccdf-xml> [-o <output-folder>] [-h] [-m <metadata-json>] [-T (rule|group|cis|version)] [-s] [-L
...
FLAGS
  ...
  -T, --idType=<option>          [default: rule] Control ID Types: 'rule' - Vulnerability IDs (ex. 'SV-XXXXX'), 'group' - Group IDs (ex. 'V-XXXXX'), 'cis' - CIS Rule IDs (ex.
...
@tab Command
``` sh
saf generate xccdf_benchmark2inspec_stub --input=resources/U_RHEL_9_STIG_V2R2_Manual-xccdf.xml --output my_rhel9_stig_profile
```
