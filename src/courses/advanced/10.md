...
In our `VALIDATE - Apply An Attestation` step, we invoke a GitHub action called `saf_action`. That action is a wrapper around the SAF CLI to make it available inside of an Actions pipeline.
...
So let's add a step to the pipeline to automatically push our results artifact to a Heimdall Server. Luckily, MITRE SAF maintains the Heimdall Demo to support this sort of use case.
...
We're going to leverate Heimdall Demo and its API to build our pipeline.
...
Note that once you add this secret, you can _overwrite_ the value, or delete it entirely, but you can't _read_ it.
