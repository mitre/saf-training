...
:::tip
To run `inspec exec` on the target, ensure that you are in the directory that has the `my_nginx` profile. If you're using the inputs file that you generated in the User course, we recommend renaming the file to `inputs-linux.yml` to match examples in the Advanced course. Additionally, if the inputs file is not in the root of the `my_nginx` directory, you will need to specify its filepath in the `--input-file` flag or move the file to the root of the profile directory.
:::
