...
- **Status**: Field indicating whether the underlying SRG requirement is applicable to the component in question. It might not be. We will discuss this in further detail when we start writing requirements in Vulcan(tm).
...
- [Heimdall(tm)](https://saf.mitre.org/apps/heimdall) (coming soon!): The MITRE SAF(tm) team is working to implement all of STIG Viewer's functionality into the Heimdall application, so that policy documents and scan data can be examined by the same application. 
:::
...
::: info STIG Authorship Challenges
