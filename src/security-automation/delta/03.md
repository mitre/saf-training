...
The [SAF(tm) CLI](https://saf-cli.mitre.org/) is a collection of libraries and tools developed by MITRE and the security community to streamline security automation for systems and DevOps pipelines. The delta process is one of the capabilities provided by the SAF CLI.
...
2. **Fuzzy matching** (optional):
   a. Maps controls based on text similarity (≥80% match threshold) instead of control IDs
...
::: details How Fuzzy Matching Works
**Example**:

Old Control (Windows 2019 - V-93283):
```
Title: "Local Administrator account must be disabled"
Description: "The built-in administrator account is a well-known..."
```

New Control (Windows 2022 - SV-268322):
```
Title: "The built-in Administrator account must be disabled"
Description: "The built-in administrator account is a well-known..."
```

Even though IDs are completely different (V-93283 vs SV-268322), the text content is 95% similar. Fuzzy matching:
1. Compares text from both controls
2. Calculates similarity score (95%)
3. Since 95% ≥ 80% threshold, maps them together
4. Copies your describe block from V-93283 to SV-268322

**Caution**: Always review fuzzy matches - similar text doesn't guarantee same requirement!
:::
