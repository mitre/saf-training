...
2. Download and extract the [latest STIG](https://public.cyber.mil/stigs/downloads/), then locate the XCCDF XML file (usually named `*-xccdf.xml`) and place it in your working directory
...
3. `saf-cli.log`: A log of the SAF(tm) CLI output from running the command
...
- Baselines belong to different platforms (e.g., RHEL → Windows)
...
## 4.2 Practice Exercise: Choosing the Right Command

Test your understanding by identifying the correct command for each scenario:

### Scenario 1
You're updating the RHEL 9 profile from V1R1 to V1R2 (minor revision update). The new STIG has legacy ID mappings and no new controls.

**Which command should you use?**
- [ ] `update_controls4delta`
- [ ] `delta` without fuzzy matching
- [ ] `delta` with fuzzy matching

### Scenario 2
The Ubuntu 22.04 CIS Benchmark added 8 new controls in version 2.0.0. Existing controls kept their IDs.
...
### Scenario 3
You're migrating an Oracle Database 12c profile to Oracle Database 19c (different major version). Control IDs completely changed with no legacy mappings.
...
**Scenario 1**: ✅ `update_controls4delta`
- **Why**: Minor revision with legacy mappings - simple rename operation
- No new controls to generate
- `update_controls4delta` is faster and sufficient

**Scenario 2**: ✅ `delta` without fuzzy matching
- **Why**: New controls need stubs generated
- `update_controls4delta` doesn't create new controls
- No need for fuzzy matching since existing IDs unchanged

**Scenario 3**: ✅ `delta` with fuzzy matching
- **Why**: No ID mappings between different major versions
- Fuzzy matching can find similar controls by text content
- Enables reuse of ~70-90% of old profile test code
:::

## 4.3 Quick Reference Table

| Scenario | Best Command | Required Flags |
|----------|--------------|----------------|
| Minor revision (IDs renamed with mappings) | `update_controls4delta` | `-X`, `-J`, `-c` |
| New controls added | `delta` | `-X`, `-J`, `-o` |
| Different platform/major version | `delta` with fuzzy matching | `-X`, `-J`, `-o`, `-M`, `-c` |
| Not sure what changed | `delta` | `-X`, `-J`, `-o` |
