...
In the [beginner class](../beginner/05.md), we wrote and ran an InSpec profile against a test container. We then generated a report on our results and loaded them into Heimdall(tm) for analysis. Let's recap this process with some practice.
...
Recall that we created a `my_nginx` InSpec profile in the [Beginner Security Automation](../beginner/index.md) class
