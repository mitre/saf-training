...
Our goal is to complete our InSpec scan, collect the result as a report file, and then parse that file to determine if we met our own *threshold* of security. We can do this with the SAF(tm) CLI.
...
- name: VERIFY - Ensure the scan meets our results threshold
  run: |
    saf validate threshold -i results/pipeline_run_attested.json -T threshold.yml
```

== `pipeline.yml` after adding verify steps
...
::: info How could we change this threshold file to ensure that the pipeline run will fail?
:::
...
From here, we can download that file and manually drop it off in something like Heimdall(tm) or feed into some other security process at our leisure. Luckily, we already automated that step. Let's check in with Heimdall Demo's Database View:
