...
Use the InSpec executable with the command `inspec exec` to run a profile against a system. The generic command is below, but take a look at our breakdown on [How To Run InSpec](../user/06.md) in the MITRE SAF(tm) User course for more information.
...
::: info What's up with CINC Auditor?
