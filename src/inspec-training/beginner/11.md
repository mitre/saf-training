...
We *really* do not want to stuck with writing all of these tags by hand. Instead, let's use the [SAF(tm) CLI](https://saf-cli.mitre.org) benchmark generator.
...
::: info "XCCDF XML" file?! That's a lot of letters!
...
You can then load the STIG document into STIG Viewer to examine the requirements in the real STIG. You could also upload that file to Vulcan(tm) for a similar view!
...
== Completed InSpec Control From the Published SAF RHEL9 Profile
